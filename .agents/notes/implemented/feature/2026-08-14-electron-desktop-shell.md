...
**A self-contained Electron app owns its whole runtime.** `apps/electron` (`@deepseek-ai/dsh-electron-app`) is the only package the desktop surface touches. The main process starts its own dsh web server as an `ELECTRON_RUN_AS_NODE` child — the checkout's built CLI (`apps/cli/lib/bin.js web --port 0`) in dev, the bundled CLI (`node_modules/@deepseek-ai/dsh/lib/bin.js web --port 0` inside the asar) when packaged — parses the `dsh web: http://127.0.0.1:<port>` readiness line for the OS-assigned URL, and couples the two lifetimes: quitting kills the server child, the server's own exit quits the app. An optional `DSH_WEB_URL` skips the embedded server for an external-harness launch. No profile, bundle, or dsh CLI change is required — `dsh web` and its web profile already exist.
