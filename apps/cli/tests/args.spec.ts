...
describe('parseDshArgs', () => {
  it('routes profile boots and the web and electron aliases, handing the rest to the app', () => {
    ...
    expect(parse(['electron', '--patch', 'shell.yml']))
      .toEqual({ mode: 'profile', profile: 'electron', patches: ['shell.yml'], args: [] })
  })
  ...
  it('routes profile and web config dumps', () => {
    ...
    expect(parse(['electron', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'electron', defaultOnly: true, patches: [] })
  })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['electron', '--dump-config', '--dump-default-config'])).toBe(1)
    expect(exitCode(['electron', '--dump-default-config', '--patch', 'e.yml'])).toBe(1)
    expect(exitCode(['electron', '--patch='])).toBe(1)
