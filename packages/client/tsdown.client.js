...
import { PLATFORM_MODULES } from "./web/src/platform.js";
/**
        ...
        plugins: [{
                ...
                resolveId(source) {
                    ...
                    if (CLIENT_EXTERNALS.includes(source))
                        return null; // platform module: external wins
                    if (VENDORED_LIBRARY.test(source))
                        return null; // vendored library: inline, no shared identity
                    if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source))
                        return null; // wire contribution: inline is the point
                    throw new Error(`client bundle purity: "${source}" is not a platform module (CLIENT_EXTERNALS), an inline-safe wire layer, or a generated /remote contribution — `
...
//# sourceMappingURL=tsdown.client.js.map